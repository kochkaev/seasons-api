...
public class Config {
    ...
    public static void init__() {
        ...
        config = new ConfigObject(TXTConfigCore.openOrCreate("Seasons/config", defaultConfig));
        String defaultCurrent = "{ \"season\": WINTER, \"weather\": NONE, \"previous-weather\": NONE, \"language\": RU_ru }";
        jsonCore = JSONConfigCore.openOrCreate("Seasons/current", defaultCurrent);
        current = jsonCore.getJsonObject();
        TXTConfigCore.createIfDoNotExists("Seasons/lang/RU_ru", DefaultTXTLangRU.init__());
        lang = new ConfigObject(TXTConfigCore.open("Seasons/lang/"+current.get("language").getAsString()));
...
class DefaultTXTConfig {
    ...
    private static void generate() {
        ...
        gen.addComment("Seconds per seasons-api tick");
