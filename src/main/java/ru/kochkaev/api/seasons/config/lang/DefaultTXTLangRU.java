...
public class DefaultTXTLangRU extends ConfigFileObject {
    ...
    public DefaultTXTLangRU() {
        super("API", "ru_RU", "lang", copyright, 2);
    }

    public void generate(ConfigContentObject content) {
        content
                .addTextValue("lang.message.seasonsModDisplayName", "<yellow><bold>Сезоны", "Seasons mod name (used in chat messages as %seasons:display-name% placeholder)")
                .addTextValue("lang.message.messageNewDay", "<yellow><bold>Наступил новый день!", "New day title message (used in titles as %seasons:title-new-day% placeholder)")
                .addTextValue("lang.message.currentInfo", "%seasons:season%. <gray>Сегодня %seasons:weather%", "Current weather and season title message (used in titles as %seasons:title-info% placeholder)");
    }
